...
class DependencyInstance(
...
) extends OldDependencyInstance(_forms, _lemmas, _cpostags, _postags, _feats, _deprels, _heads, _relFeats) with Serializable {
  def this(
  forms: Array[String],
  lemmas: Array[String],
  ...
  def difference(that: DependencyInstance) = if (this.size == that.size) Some({
    ...
    //this.heads.zip(that.heads).tail.filter(Function.tupled(_ != _)).size,
    //this.heads.zip(this.deprels).zip(deprels.zip(that.deprels).tail.filter(Function.tupled(_ != _)).size
  }) else None
}
